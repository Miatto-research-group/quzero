...
def softmax_sample(visit_counts: List[Tuple[int, Action]], temperature: float) -> Action:
  tot_visits = sum(visits for visits, action in visit_counts)
  distribution = np.array([visits / tot_visits for visits, action in visit_counts])
  choice = np.random.choice(len(distribution), p=(distribution ** temperature) / sum(distribution ** temperature))
  return 0, visit_counts[choice][1]
...
def expand_node(node: Node, to_play: Player, actions: List[Action], network_output: NetworkOutput):
    ...
    policy = tf.math.exp(network_output.policy_logits)
    policy /= tf.reduce_sum(policy)
    for action in actions:
        node.children[action] = Node(policy[action.index])
...
def backpropagate(search_path: List[Node], value: float, to_play: Player, discount: float, min_max_stats: MinMaxStats):
    for node in reversed(search_path):
        node.value_sum += value if node.to_play == to_play else -value #TODO: how are players compared?
        node.visit_count += 1
